...
Working directory: ${params.SOURCE_DIRECTORY}
                    ...
                    if ("${BRANCH}" =~ /^(master|develop|feature*)/) {
                        ...
                        if (fileExists("${params.SOURCE_DIRECTORY}/laradock/${COMPOSE_YML}")) {
                            ...
                            echo "--> Deploying"
                            sh "env \$(cat ${params.SOURCE_DIRECTORY}/laradock/.my-custom-env | grep ^[A-Z] | xargs) docker stack deploy -c ${params.SOURCE_DIRECTORY}/laradock/${COMPOSE_YML} --prune ${NAME}-${BRANCH}"
                        } else {
